...
COLUMNS = ["Payment Term Name", "Description", "Due Date Based On", "Credit Days", "Invoice Portion"]
EXPECTED_COLUMNS = 5
    ...
    if not cint(for_validate):
        ...
        for row in data:
            preview_data.append({
                ...
                "invoice_portion": flt(row[4]) if len(row) > 4 else 100,
            })
        return preview_data
    else:
...
def download_template(file_type: str):
    ...
    sample_rows = [
        ["Net 30", "Payment due in 30 days", "Day(s) after invoice date", "30", "100"],
        ["Net 60", "Payment due in 60 days", "Day(s) after invoice date", "60", "100"],
        ["Due on Receipt", "Payment due immediately", "Day(s) after invoice date", "0", "100"],
        ["End of Month", "Payment due at end of month", "Day(s) after the end of the invoice month", "0", "100"],
        ["Net 30 EOM", "30 days after end of invoice month", "Month(s) after the end of the invoice month", "1", "100"],
    ]
